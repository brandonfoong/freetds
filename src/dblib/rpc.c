...
insert_values_into_table(TDSSOCKET * tds, TABLE_VALUE * table, const char paramname[], TDS_SERVER_TYPE type, DBINT datalen[], BYTE * value, DBINT num_cols) {
	...
	/* Add on the column metadata */
	for (pmetadata = &table->metadata; *pmetadata != NULL; pmetadata = &(*pmetadata)->next)
		continue;

	*pmetadata = metadata;

	for (i = 0, row = table->row; row != NULL; row = row->next, i++) {
		params = row->params;
		 ...
		 */
		temp_value = is_char_type(type) ? ((BYTE **) value)[i] : value + offset;
		temp_datalen = datalen[i];
		...
		/* meta data */
		if (paramname)
			if (!tds_dstr_copy(&pcol->column_name, paramname)) {
				tds_free_param_results(params);
		...
		if (is_fixed_type(type)) {
			pcol->column_size = tds_get_size_by_type(type);
		} else {
			pcol->column_size = datalen[i];
		}
		if (type == XSYBNVARCHAR)
			pcol->column_size *= 2;
		pcol->on_server.column_size = pcol->column_size;

		pcol->column_output = 0; /* Status value set to 0 - strictly only input param */
		pcol->column_cur_size = temp_datalen;

		prow = param_row_alloc(params, pcol, num_cols, temp_value, temp_datalen);
...
dbrpcbindcolumn(DBPROCESS * dbproc, TABLE_VALUE * table, const char paramname[], int type, DBINT datalen[], BYTE * value)
{
	...
	if (!insert_values_into_table(dbproc->tds_socket, table, paramname, type, datalen, value, table->num_cols))
		return FAIL;
